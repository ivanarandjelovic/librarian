...
public class BookControllerUTest {
  ...
  public void testDeleteBook() {
    ...
    UnitTestData.testBooks[0].setTitle(newTestBookTitle);
    Model model = new ExtendedModelMap();
    String view = bookController.deleteBook(UnitTestData.testBooks[0].getId(), model);
    assertEquals("delete book view is wrong", "delete_book", view);
  }

  @Test
   public void testDeleteBookDone() {

     String newTestBookTitle = "test book title " + System.currentTimeMillis();
     UnitTestData.testBooks[0].setTitle(newTestBookTitle);
     Model model = new ExtendedModelMap();
     String view = bookController.deleteBookConfirm(UnitTestData.testBooks[0].getId(), model);
     assertNull("Book should have been deleted!", bookDaoMock.getBooks().get(UnitTestData.testBooks[0].getId()));
     assertEquals("delete book view is wrong", "delete_book_done", view);
   }
}
